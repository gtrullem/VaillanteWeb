...
if (isset($Submit)) {
	...
	if ($datefin=="") { // On considère qu'on a une date de fin
	...
	if($OK) {
		...
		$result = mysql_db_query($dbname,$request,$connect) or die('SQL : GET_PRESTATION !<br>'.$result.'<br>'.mysql_error());
		$exportxls = $result;
		...
		while($rowxls = mysql_fetch_array($exportxls)){
			...
			$worksheet->write($numligne, 1, $rowxls[nodossier], $format_info);
...
//			$worksheet->writeFormula($numligne, 10, "=SUM(J2:J$nbrow)",$format_total);
	...
	/**************************************************************** DOSSIER ****************************************************************/
		...
		$worksheet->setColumn(0,0,2);			// Espace
